...
def validate(model, val_data_loader, iter, device, criterion, dataset_size, writer, is_ordinal_reg, group):
    ...
    for i, batch in enumerate(val_data_loader):
        ...
        with torch.no_grad():
            classification_logits, preds = model(inputs)
        # import pdb
        # pdb.set_trace()
        ...
        for batch in tqdm(data_loaders[phase]):
            if iter % validate_at_k == 0 and iter != 0:
                ...
                for group in ['val_apref_ds']: #['val_qtst_rtst', 'val_qtst_rtrn', 'val_apref_ds']:
                    val_accuracy = -1 #
                    #val_accuracy = validate(model, data_loaders[group], iter, device, criterion, dataset_sizes[group], writer, is_ordinal_reg, group)
