...
class DiffPipeline4Model(nn.Module):
	def __init__(self, 
					age_interval,
					min_age,
					max_age,
		...
		super(DiffPipeline4Model, self).__init__()
...
class DiffPipeline4DeepModel(nn.Module):
	DEEP2 = True
	def __init__(self, 
					age_interval,
					min_age,
					...
					num_references=1):
		super(DiffPipeline4DeepModel, self).__init__()
		self.device = device
		self.age_interval = age_interval
...
class AgeDiffModel(nn.Module):
	...
	def __init__(self, 
		...
		if self.config_type == DiffModelConfigType.MlpHead:
			...
			k = self.num_features_diff_emb #1024 
			#k = int(0.125*self.num_features_diff_emb)
			self.fc_first_stage = Linear(self.num_references*self.num_features_diff_emb, k) # num_features_emb_head
			# self.fc_first_stage = Linear(self.num_references*self.num_features_diff_emb, int(0.5*self.num_references*self.num_features_diff_emb)) # num_features_emb_head
			# self.fc_sec_stage = Linear(int(0.5*self.num_references*self.num_features_diff_emb), int(0.25*self.num_references*self.num_features_diff_emb)) 
			# self.fc_third_stage = Linear(int(0.25*self.num_references*self.num_features_diff_emb), k) 
		else:
			print("bad config type")
			exit()
	...
	def forward(self, input_images, p=0.5):
		...
		# first stage
		class_embed = self.fc_first_stage(packed_embs_b)
		# class_embed = F.leaky_relu(class_embed)
		# class_embed = Dropout(p)(class_embed)
		# class_embed = self.fc_sec_stage(class_embed)
		# class_embed = F.leaky_relu(class_embed)
		# class_embed = Dropout(p)(class_embed)
		# class_embed = self.fc_third_stage(class_embed)
		# class_embed = F.leaky_relu(class_embed)
		# class_embed = Dropout(p)(class_embed)
		# class_embed = F.normalize(class_embed, dim=1, p=2)
		#x = F.relu(class_embed)
