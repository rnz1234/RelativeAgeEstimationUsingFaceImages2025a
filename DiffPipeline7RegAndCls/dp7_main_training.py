...
from Common.Losses.OrdinalMeanVarianceLoss import OrdinalMeanVarianceLoss


import dp7_config as cfg

from dp7_dataset import AgeDiffSameUniformDiffDataset, AgeDiffMixedUniformDiffDataset, AgeDiffMimicDiffDataset, get_error_constrained_dataset
from Common.Datasets.Morph2.data_parser import DataParser
from dp7_train import train_unified_model_iter


from dp7_model import AgeDiffRegAndClsModel #, DiffModelConfigType
...
if __name__ == "__main__":
	...
	if cfg.TASK_TYPE == "train_range0to10_test_all":
		radius_for_test_set_filter = cfg.AGE_RADIUS
	else:
		radius_for_test_set_filter = cfg.APREF_VLD_SET_AGE_DIFF_LEARN_RADIUS_HI
	...
	x_test_filtered, y_test_filtered, batst_set_filtered_indexes = get_error_constrained_dataset(orig_dataset_images=x_test, 
																									...
																									age_diff_learn_radius_hi=radius_for_test_set_filter,#cfg.AGE_DIFF_LEARN_RADIUS_HI,
	...
	if cfg.TASK_TYPE == "train_range0to10_test_all":
		print("Testing (q tst where AgePredict(q), r trn where age(r)=AgePredict(q)) set size: " + str(len(test_apref_ds)))
	else:
	...
	dataset_sizes = {x: len(image_datasets[x]) for x in ['train', 'val']}#, 'val_apref_all_ds']}
	...
	data_loaders = {
		...
		'val': DataLoader(test_apref_ds, batch_size=cfg.BATCH_SIZE, num_workers=cfg.NUM_OF_WORKERS_DATALOADER_VLD, pin_memory=True, shuffle=False, drop_last=True),
	...
	#           Model
	#####################################################
	num_classes = int((2*cfg.AGE_DIFF_LEARN_RADIUS_HI) / cfg.AGE_INTERVAL + 1)
	model = AgeDiffRegAndClsModel(age_diff_interval=cfg.AGE_INTERVAL, 
								min_age_diff=-cfg.AGE_DIFF_LEARN_RADIUS_HI, 
								max_age_diff=cfg.AGE_DIFF_LEARN_RADIUS_HI, 
	...
	if cfg.SCHEDULER == "CosineAnnealingLR+GradualWarmupScheduler":
		...
		if cfg.DISABLE_WARMUP:
			...
			print("scheduler: using CosineAnnealingLR")
		else:
		...
		reduce_lr_on_plateau_scheduler = ReduceLROnPlateauEnhanced(optimizer, mode='min', factor=cfg.REDUCE_LR_ON_PLATEAU_FACTOR, patience=cfg.REDUCE_LR_ON_PLATEAU_PATIENCE, min_lr=cfg.REDUCE_LR_ON_PLATEAU_MIN_LR)
		# import pdb
		# pdb.set_trace()
		if cfg.DISABLE_WARMUP:
	...
	shutil.copyfile("dp7_config.py", model_path + "/dp7_config.py")
	shutil.copyfile("dp7_dataset.py", model_path + "/dp7_dataset.py")
	shutil.copyfile("dp7_model.py", model_path + "/dp7_model.py")
	shutil.copyfile("dp7_train.py", model_path + "/dp7_train.py")
	shutil.copyfile("dp7_main_training.py", model_path + "/dp7_main_training.py")
	#shutil.copyfile("dp7_main_training.ipynb", model_path + "/dp7_main_training.ipynb")
	#shutil.copyfile("dp7_main_diff_inference.ipynb", model_path + "/dp7_main_diff_inference.ipynb")
	#shutil.copyfile("dp7_main_e2e_inference.ipynb", model_path + "/dp7_main_e2e_inference.ipynb")
	...
	if cfg.IS_ORDINAL:
		mean_var_criterion = OrdinalMeanVarianceLoss(0, num_classes, device, lambda_mean=0.2, lambda_variance=0.05).to(device)
	else:
