...
class AgeDiffSameUniformDiffDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {'image_vec': image_vec, 'label': pair_label, 'age_diff': age - ref_image_arr_age[0], 'age_ref': ref_image_arr_age[0], 'age_query' : age}
...
class AgeDiffMimicDiffDataset(Dataset):
    ...
    def _find_ref_image(self, idx):
        ...
        if DEBUG_REF_SIMILARITY:
            ...
            q_r_distance = np.linalg.norm(self.embs_vld_rsh[idx] - self.embs_trn_rsh[actual_idxs], axis=1)
            print(f'q-r distance: {q_r_distance}')
            plt.show()
