    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(data_loaders[phase]):
            if iter % validate_at_k == 0 and iter != 0:
                ...
                model.eval()
                #diff_pipeline_confusion_matrix_analysis(model, data_loaders['train'], device, dataset_sizes['train'], is_ordinal_reg=True, is_pipeline=False)
                for group in ['val_qtst_rtrn', 'val_apref_ds']: #['val_qtst_rtst', 'val_qtst_rtrn', 'val_apref_ds']:
        ...
        if validate_at_end_of_epoch:
            ...
            for group in ['val_qtst_rtrn', 'val_apref_ds']: #['val_qtst_rtst', 'val_qtst_rtrn', 'val_apref_ds']:
                #for group in ['val_apref_all_ds']:                 
                val_mae_hard, val_mae_hard_final, val_mae_soft = validate(model, data_loaders[group], iter, device, criterion, dataset_sizes[group], writer, include_accuracy_metrics, is_ordinal_reg, group)
                diff_pipeline_confusion_matrix_analysis(model, data_loaders[group], device, dataset_sizes[group], is_ordinal_reg=True, is_pipeline=False, min_class=min_class, max_class=max_class)
