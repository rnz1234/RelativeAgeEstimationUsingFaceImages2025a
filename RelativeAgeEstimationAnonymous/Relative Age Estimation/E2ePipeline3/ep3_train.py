...
from ep3_config import USE_GENDER, LEARN_MINUS_DIFF_CLS
...
def evaluate(
	...
	criterion_cls,
	criterion_mean_var,
	criterion_cls_main_reg,
	criterion_mean_var_main_reg,
	criterion_gender_est,
	criterion_ordinal_mean_var_side,
	num_references,
	writer,
	model_path,
	epoch,
	...
	for batch in tqdm(dataloader):
		...
		if USE_GENDER:
			gender_label = batch['gender'].to(device).float()
		...
		with torch.no_grad():
			...
			if USE_GENDER:
				if regressors_diff_head:
					...
					age_pred_f, age_pred_r, age_diff_preds_f, age_diff_preds_r, classification_logits, age_diff_pred_cls, classification_logits_main_diff, classification_logits_main_diff_minus, gender_head_cls_pre_sigmoid = model(input_images=image_vec, query_noisy_age=query_age_noised, input_ref_ages=age_refs) 
				else:
				...
				if regressors_diff_head:
					#age_pred_f, age_pred_r, age_diff_preds_f, age_diff_preds_r, classification_logits, classification_logits_minus, age_diff_pred_cls = model(input_images=image_vec, query_noisy_age=query_age_noised, input_ref_ages=age_refs)
					age_pred_f, age_pred_r, age_diff_preds_f, age_diff_preds_r, classification_logits, age_diff_pred_cls, classification_logits_main_diff, classification_logits_main_diff_minus = model(input_images=image_vec, query_noisy_age=query_age_noised, input_ref_ages=age_refs) 
				else:
					#age_pred_f, age_pred_r, age_diff_preds_f, age_diff_preds_r, classification_logits, classification_logits_minus = model(input_images=image_vec, query_noisy_age=query_age_noised, input_ref_ages=age_refs)
			...
			if is_ordinal:
				...
				for i in range(age_diff_cls_labels.shape[1]):
					levels = levels_from_labelbatch(age_diff_cls_labels[:, i].int(), num_classes=num_classes_diff).to(device)
				...
				if criterion_ordinal_mean_var_side is not None:
					loss_mean_var_side = criterion_ordinal_mean_var_side(classification_logits.view(classification_logits.shape[0]*classification_logits.shape[1], classification_logits.shape[2]), age_diff_cls_labels.view(age_diff_cls_labels.shape[0]*age_diff_cls_labels.shape[1]))
				else:
					loss_mean_var_side = 0.0
			...
			# 	loss_mean, loss_var = criterion_mean_var(classification_logits.view(classification_logits.shape[0]*classification_logits.shape[1], classification_logits.shape[2]), age_diff_cls_labels.view(age_diff_cls_labels.shape[0]*age_diff_cls_labels.shape[1]))

			total_loss = age_loss + age_diff_loss + loss_cls + loss_mean_var_side
			# if criterion_mean_var is not None:
			# 	total_loss += loss_mean + loss_var

			
			# main diff regressors
			total_loss += criterion_cls_main_reg(classification_logits_main_diff.view(classification_logits_main_diff.shape[0]*classification_logits_main_diff.shape[1], classification_logits_main_diff.shape[2]),  age_diff_cls_labels.view(age_diff_cls_labels.shape[0]*age_diff_cls_labels.shape[1]).long())
			if LEARN_MINUS_DIFF_CLS:
				total_loss += criterion_cls_main_reg(classification_logits_main_diff_minus.view(classification_logits_main_diff_minus.shape[0]*classification_logits_main_diff_minus.shape[1], classification_logits_main_diff_minus.shape[2]),  age_minus_diff_cls_labels.view(age_minus_diff_cls_labels.shape[0]*age_minus_diff_cls_labels.shape[1]).long())
			loss_mean_mdh, loss_var_mdh = criterion_mean_var_main_reg(classification_logits_main_diff.view(classification_logits_main_diff.shape[0]*classification_logits_main_diff.shape[1], classification_logits_main_diff.shape[2]),  age_diff_cls_labels.view(age_diff_cls_labels.shape[0]*age_diff_cls_labels.shape[1]))
			total_loss += loss_mean_mdh + loss_var_mdh

			# metadata (gender, etc.)
			if USE_GENDER:
				gender_est_head_flat = gender_head_cls_pre_sigmoid.view(-1)
				gender_loss = criterion_gender_est(gender_est_head_flat, gender_label)
				total_loss += gender_factor*gender_loss
			...
			if USE_GENDER:
				running_loss_gender += gender_loss.item() * image_vec.size(0)
	...
	if USE_GENDER:
		epoch_loss_gender = running_loss_gender / dataset_size
	...
	if USE_GENDER:
		writer.add_scalar(f'GenderLoss/{set_type}', epoch_loss_gender, epoch)
		...
		for batch in tqdm(dataloaders['train']):
			...
			with autocast():
				...
				if is_ordinal:
					loss_cls = 0.0
					for i in range(age_diff_cls_labels.shape[1]):
						levels = levels_from_labelbatch(age_diff_cls_labels.int()[:, i], num_classes=num_classes_diff).to(device)
						loss_cls += condor_negloglikeloss_ext(classification_logits[:,i,:], levels, device)
					loss_cls = loss_cls / age_diff_cls_labels.shape[1]
					...
					# loss_cls_minus = 0.0
					# for i in range(age_diff_cls_labels.shape[1]):
					# 	levels = levels_from_labelbatch(age_minus_diff_cls_labels.int()[:, i], num_classes=num_classes_diff).to(device)
					# 	loss_cls_minus += condor_negloglikeloss_ext(classification_logits_minus[:,i,:], levels, device)
					# loss_cls_minus = loss_cls_minus / age_minus_diff_cls_labels.shape[1]
			...
			#valid_iter = int(10+(dataset_sizes['train']/image_vec.size(0))/2)
			# if iterat % valid_iter == 0:
			# 	print("---- validation")
			...
			# 	if val_age_mae_r < best_mae:
			# 		best_mae = val_age_mae_r 
			# 		best_model_wts = copy.deepcopy(model.state_dict())
			# 		if remove_older_checkpoints:
			# 			if model_file_path is not None:
			# 				print("removing previous weights")
		...
		if val_age_mae_r < best_mae:
			...
			if remove_older_checkpoints:
				if model_file_path is not None:
					print("removing previous weights")
					os.system("rm -rf {}".format(model_file_path))
			model_file_path = os.path.join(model_path, "weights_{}_{:.4f}.pt".format(epoch, val_age_mae_r))
			if save_all_model_metadata:
				torch.save({
					'model_state_dict' : best_model_wts,
					'optimizer_state_dict': copy.deepcopy(optimizer.state_dict()),
