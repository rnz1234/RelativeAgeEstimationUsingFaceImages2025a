...
class AgeDiffModel(nn.Module):
	DEEP2 = False#True #True 
	...
	def __init__(self, 
		...
		if self.DEEP2:
			...
			#self.diff_embedding_layer3 =  Linear(int(1.5*self.num_features_diff_emb),  int(1.2*self.num_features_diff_emb))
		...
		if self.is_ordinal_reg:
			num_of_classes = 2*age_diff_learn_radius_hi
		else:
			num_of_classes = 2*age_diff_learn_radius_hi+1

		if self.deep:
			m = self.fc_2nd_layer_size
			self.fc_second_stage = Linear(k, m)
			self.class_head = Linear(m, num_of_classes)
			self.in_range_head = Linear(m, 1)
		else:
			self.class_head = Linear(k, num_of_classes)
			self.in_range_head = Linear(k, 1)
	...
	def forward(self, input_images, input_ref_age, p=0.5):
		...
		if agg_type == "fc":
			...
			x = self.aggregation_stage(packed_embs_a.transpose(1,2))
			x = x.view(x.shape[0], x.shape[1])
		else:
			print("error in agg_type")
			exit()
		...
		is_in_range_est = self.in_range_head(x)
		...
		if self.is_ordinal_reg:
			...
			age_diff_pred_soft = torch.matmul(predicted_probs.to(self.device), torch.Tensor(list(range(-self.age_diff_learn_radius_hi,self.age_diff_learn_radius_hi+1))).to(self.device))
			
		else:
			weights = nn.Softmax()(classification_logits)

			_, preds = torch.max(classification_logits, 1)
			age_diff_pred_hard = preds - self.age_diff_learn_radius_hi
		...
		alpha = nn.Sigmoid()(is_in_range_est)
		final_age_est = (1-alpha).view(-1).to(self.device)*input_ref_age.to(self.device) + alpha.view(-1).to(self.device)*(input_ref_age.to(self.device)+age_diff_pred_soft.to(self.device))




		return classification_logits, age_diff_pred_hard, age_diff_pred_soft, is_in_range_est, final_age_est