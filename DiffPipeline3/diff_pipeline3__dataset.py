...
class DiffPipeline3Dataset(Dataset):
    def __init__(self,
        ...
        self.enhanced_diff_train_set = enhanced_diff_train_set
    ...
    def _find_ref_image(self, idx):
        ...
        if self.dataset_type == "train":
            query_est_age = self.ages_ba_train[idx] + np.random.normal(0, 3.38)
        else:
            query_est_age = self.im2age_map_batst[str(idx)]
    ...
    def __len__(self):
        if self.dataset_type == "train":
            return len(self.batrn_set_metadata)
        else:
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset_type == "train":
            orig_image = self.batrn_set_images[idx]
        else:
        ...
        if self.dataset_type == "train":
            metadata = json.loads(self.batrn_set_metadata[idx])
        else:
